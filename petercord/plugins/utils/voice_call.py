...
@petercord.on_cmd("leavevc", about={
    'header': "Leave Voice-Call",
    'petercord': "{tr}leavevc"},
    allow_private=False)
@vc_chat
...
@petercord.on_cmd("play", about={'header': "play or add songs to queue"},
...
if petercord.has_bot:
    @petercord.bot.on_callback_query(filters.regex("(skip|queue|back)"))
    @check_cq_for_all
    ...
    @petercord.bot.on_callback_query(filters.regex(r"vol\((.+)\)"))
    @check_cq_for_all
    async def vol_callback(cq: CallbackQuery):
        ...
        if arg.isnumeric():
            ...
            try:
                async with petercord.conversation(cq.message.chat.id, user_id=cq.from_user.id) as conv:
