...
@petercord.on_cmd(
    "help", about={"header": "Guide to use PETERCORD commands"}, allow_channels=False
)
async def helpme(
    message: Message,
    ...
    if not message.input_str:
        ...
        out_str += f"""📕 <b>petercord:</b>  <code>{Config.CMD_TRIGGER}help [plugin_name]</code>"""
    else:
    ...
    def check_owner(func):
        async def wrapper(_, c_q: CallbackQuery):
            if c_q.from_user and c_q.from_user.id in Config.OWNER_ID:
                ...
                await c_q.answer(
                    f"Only {user_dict['flname']} Can Access this...! Build Your Own @petercord 🤘",
                    show_alert=True,
                )
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            ...
                            InlineKeyboardButton(
                                "🖥 Deploy Userge", url="https://t.me/petercord"
                            ),
                        ]
                    ]
                ),
